...
const { randomDigit, extractDetails, findOperatorCode, parseMessage } = require('./function');
...
app.post('/wa-recharge', async (req, res) => {
  ...
  try {
    ...
    let operator_code;
    let circle_code;
    const { number, operator, circle, amount } = parseMessage(m_content);
    const usertx = randomDigit()
    const operatorCode = findOperatorCode(operator);
    ...
    if(!number || !amount){
      const newResponse =  await sendMessageToWhatsApp({
        message:`Please provide valid number and amount`,
        phone:m_phone
      })
      console.log(newResponse,"Please provide valid number and amount Message");
      res.status(402).json({
        status: true,
        message: `Please provide valid number and amount`,
        data: newResponse,
...
app.post('/wa-recharge-old', async (req, res) => {
...
//FAILURE
    ...
    const { TransID,Status ,Amount:amount,OperatorRef,ErrorCode} = req.query

  
    // if(Status=='FAILURE'){ 
    //   const newResponse =  await sendMessageToWhatsApp({
    //      message:`Recharge of ${amount} rupees ${ErrorCode} for number ${number} with trasaction ref ${TransID}.`,
    //      phone:m_phone
    //    })
    //    console.log(newResponse,"Send  Success Message");
    //    res.status(200).json({
    ...
    //    res.status(200).json({
    //      status: true,
    //      message: `Recharge of ${amount} rupees ${ErrorCode} for trasaction ${TransID}`,
    //      data: ErrorCode,
    //    });
    //  }
